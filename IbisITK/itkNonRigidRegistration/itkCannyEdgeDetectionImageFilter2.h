...
#ifndef __itkCannyEdgeDetectionImageFilter2_h
#define __itkCannyEdgeDetectionImageFilter2_h
...
class ITK_EXPORT CannyEdgeDetectionImageFilter2:
  public ImageToImageFilter< TInputImage, TOutputImage >
{
public:
  /** Standard "Self" & Superclass typedef.  */
  typedef CannyEdgeDetectionImageFilter2                   Self;
  ...
  itkTypeMacro(CannyEdgeDetectionImageFilter2, ImageToImageFilter);
  ...
  typedef   itk::Statistics::ListSample< MeasurementVectorType >   
                                                         GradientMagnitudeSampleType;

  typedef   itk::Statistics::Histogram <
      float, itk::Statistics::DenseFrequencyContainer2 >    HistogramType;

  typedef   itk::Statistics::SampleToHistogramFilter <
              GradientMagnitudeSampleType, HistogramType >  SampleToHistogramFilterType;
   ...
   * the CannyEdgeDetectionImageFilter2, and that both
  ...
  itkSetMacro( UsePercentiles, bool );
  itkGetConstReferenceMacro( UsePercentiles, bool );
  itkBooleanMacro( UsePercentiles );   

  itkSetMacro(UpperPercentile, OutputImagePixelType);
  itkGetConstMacro(UpperPercentile, OutputImagePixelType);

  itkSetMacro(LowerPercentile, OutputImagePixelType);
  itkGetConstMacro(LowerPercentile, OutputImagePixelType);  


  itkSetObjectMacro(MaskImage, OutputImageType);
  ...
  /** CannyEdgeDetectionImageFilter2 needs a larger input requested
   * region than the output requested region ( derivative operators, etc).
   * As such, CannyEdgeDetectionImageFilter2 needs to provide an implementation
...
#endif
protected:
  CannyEdgeDetectionImageFilter2();
...
private:
  CannyEdgeDetectionImageFilter2(const Self &); //purposely not implemented
  void operator=(const Self &); //purposely not implemented

  virtual ~CannyEdgeDetectionImageFilter2(){}

  /** Thread-Data Structure   */
  struct CannyThreadStruct
  {
    CannyEdgeDetectionImageFilter2 *Filter;
  };
  ...
  bool      m_UsePercentiles;

  /** Upper threshold value for identifying edges. */
  OutputImagePixelType m_UpperPercentile;  //should be float here?

  /** Lower threshold value for identifying edges. */
  OutputImagePixelType m_LowerPercentile; //should be float here?
  ...
  typename OutputImageType::Pointer     m_MaskImage;
};
} //end of namespace itk

#ifndef ITK_MANUAL_INSTANTIATION
#include "itkCannyEdgeDetectionImageFilter2.hxx"
#endif
