...
class CommandIterationUpdateStandaloneOpenCL : public itk::Command
{
public:
  typedef  CommandIterationUpdateStandaloneOpenCL   Self;
  typedef  itk::Command             Superclass;
  typedef itk::SmartPointer<Self>  Pointer;
  itkNewMacro( Self );

protected:
  CommandIterationUpdateStandaloneOpenCL() {};
...
void GPURigidRegistration::runRegistration()
{
    ...
    CommandIterationUpdateStandaloneOpenCL::Pointer observer = CommandIterationUpdateStandaloneOpenCL::New();
    observer->SetVtkTransform( sourceVtkTransform );
    ...
    ItkRigidTransformType::MatrixType matrix3x3 = itkTransform->GetMatrix();
    ItkRigidTransformType::OffsetType translation = itkTransform->GetOffset();
    vnl_matrix<double> vnlMatrix = matrix3x3.GetVnlMatrix();

    for (int i = 0; i < 3 ; ++i) {
        if (i < 3)
            vtkMatrix->SetElement(i, 3, translation.GetElement(i));
        for (int j = 0; j < 3; ++j) {
            vtkMatrix->SetElement(i, j, vnlMatrix[i][j]);
        }
    }

    vtkTransform * resultTransform = vtkTransform::New();
    resultTransform->SetMatrix(vtkMatrix);

    m_resultTransform = resultTransform;

}


vtkTransform * GPURigidRegistration::GetVtkTransformFromItkImage(IbisItkFloat3ImageType::Pointer itkImage, bool load_translation)
{
