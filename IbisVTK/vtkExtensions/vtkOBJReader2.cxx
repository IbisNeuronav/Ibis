        ...
        vtkInformationVector *outputVector)
{
    ...
    vtkUnsignedCharArray * vertexColors = vtkUnsignedCharArray::New();
    ...
    { // (make a local scope section to emphasise that the variables below are only used here)
        ...
        while (everything_ok && fgets(rawLine, MAX_LINE, in) != NULL)
        {
            ...
            if (strcmp(cmd, "v") == 0)
            {
                ...
                int nbElementsRead = sscanf(pLine, "%f %f %f %f %f %f", xyz, xyz+1, xyz+2, xyz+3, xyz+4, xyz+5 );
                if( nbElementsRead == 3 || nbElementsRead == 6 )
                {
                    points->InsertNextPoint(xyz);
                    if( nbElementsRead == 6 )
                    {
                        hasVertexColor = true;
                        vertexColors->InsertNextTuple3( xyz[3]*255, xyz[4]*255, xyz[5]*255 );
                    }
                }
                else
                {
    ...
    if (everything_ok)   // (otherwise just release allocated memory and return)
    {
        ...
        // if some vertices have colors, assign the color array to the points data
        if( hasVertexColor )
            output->GetPointData()->SetScalars( vertexColors );
...
void vtkOBJReader2::PrintSelf(ostream& os, vtkIndent indent)
{
