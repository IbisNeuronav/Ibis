...
vtkIbisGLSLVolumeRaycastMapper::vtkIbisGLSLVolumeRaycastMapper()
{
    ...
    this->MinFogDistance = 100.0;
    this->MaxFogDistance = 1000.0;
...
void vtkIbisGLSLVolumeRaycastMapper::Render( vtkRenderer * ren, vtkVolume * vol )
{
    ...
    res = VolumeShader->SetVariable( "fogStart", float(MinFogDistance) );
    res = VolumeShader->SetVariable( "fogEnd", float(MaxFogDistance) );
    int enableFog = FogEnabled ? int(1) : int(0);
    res = VolumeShader->SetVariable( "isFogEnabled", enableFog );
    ...
    res = VolumeShader->SetVariable( "cameraPosTSpace", (float)cameraPos[0], (float)cameraPos[1], (float)cameraPos[2] );
...
{
    // VTK6
    //vtkAlgorithmOutput * in = 0;
    //    if( im )
    //        in = im->GetProducerPort();
    //int index = this->GetNumberOfInputConnections(0);
    //this->SetNumberOfInputConnections( 0, index + 1 );
    //this->SetNthInputConnection( 0, index, in );
