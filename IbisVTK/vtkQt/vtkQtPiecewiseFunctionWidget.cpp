...
vtkQtPiecewiseFunctionWidget::vtkQtPiecewiseFunctionWidget( QWidget * parent ) : QWidget( parent ), m_function( 0 )
{
    m_xRange[ 0 ]      = 0.0;
    m_xRange[ 1 ]      = 255.0;
    m_yRange[ 0 ]      = 0.0;
...
{
    if( m_function ) m_function->UnRegister( 0 );
}
...
void vtkQtPiecewiseFunctionWidget::SetFunction( vtkPiecewiseFunction * func )
{
    if( m_function ) m_function->UnRegister( 0 );
    m_function = func;
    if( m_function ) m_function->Register( 0 );
}
...
void vtkQtPiecewiseFunctionWidget::paintEvent( QPaintEvent * event )
{
    ...
    // Draw the graph
    if( m_function ) drawGraph( p, palette().color( QPalette::WindowText ) );
...
{
    Q_ASSERT_X( m_function, "vtkQtPiecewiseFunctionWidget::drawGraph()",
                "m_function needs to be defined before calling this" );

    painter.setPen( drawColor );
    painter.setBrush( drawColor );

    // need at least 1 point to draw something
    if( m_function->GetSize() < 1 ) return;
    ...
    for( int i = 1; i < m_function->GetSize(); ++i )
    {
        ...
        painter.drawRect( point[ 0 ] - m_pointRadius, point[ 1 ] - m_pointRadius, 2 * m_pointRadius,
                          2 * m_pointRadius );
    }
}
...
int vtkQtPiecewiseFunctionWidget::PointIndexFromPixCoord( int x, int y )
{
    ...
    if( minDist < m_pointRadius + 1 ) return minIndex;
    return -1;
}

void vtkQtPiecewiseFunctionWidget::GetPixCoordFromNodeCoord( int index, int point[ 2 ] )
{
    Q_ASSERT_X( m_function, "vtkQtPiecewiseFunctionWidget::GetPixCoordFromNodeCoord()",
                "m_function needs to be defined before calling this" );
...
{
    Q_ASSERT_X( m_function, "vtkQtPiecewiseFunctionWidget::SetNodeCoordFromPixCoord()",
                "m_function needs to be defined before calling this" );
    ...
    if( index > 0 )
    {
        ...
        if( newX < prevCoord[ 0 ] ) newX = prevCoord[ 0 ];
    }
    if( index < m_function->GetSize() - 1 )
    {
        double nextCoord[ 4 ];
        m_function->GetNodeValue( index + 1, nextCoord );
        if( newX > nextCoord[ 0 ] ) newX = nextCoord[ 0 ];
    }
...
void vtkQtPiecewiseFunctionWidget::NodeCoordFromPixCoord( int x, int y, double & xNode, double & yNode )
{
    ...
    if( xNode > m_xRange[ 1 ] ) xNode = m_xRange[ 1 ];
    if( xNode < m_xRange[ 0 ] ) xNode = m_xRange[ 0 ];

    yNode = m_yRange[ 0 ] + ( height() - y - 1 ) * ( m_yRange[ 1 ] - m_yRange[ 0 ] ) / height();
    if( yNode > m_yRange[ 1 ] ) yNode = m_yRange[ 1 ];
    if( yNode < m_yRange[ 0 ] ) yNode = m_yRange[ 0 ];
}
