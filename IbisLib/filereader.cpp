...
FileReader::FileReader( QObject * parent ) : QThread( parent )
...
FileReader::~FileReader()
{
    ...
    if( m_params && m_selfAllocParams ) delete m_params;
}

static QStringList mincConvertPaths = QStringList() << "/usr/bin/mincconvert"
...
static QStringList mincCalcPaths = QStringList() << "/usr/bin/minccalc"
...
bool FileReader::HasMincConverter() { return !m_mincconvert.isEmpty(); }
...
bool FileReader::ConvertMINC1toMINC2( QString & inputileName, QString & outputileName, bool isVideoFrame )
{
    ...
    QDir tmpDir( dirname );
    bool ok = true;
    if( !tmpDir.exists() ) ok = tmpDir.mkdir( dirname );
    if( !ok )
    {
    ...
    if( !isVideoFrame )
    {
        ...
        if( ok ) ok = convertProcess->waitForFinished();
    ...
    else
    {
        ...
        if( ok ) ok = convertProcess->waitForFinished( 5000 );
        delete convertProcess;
        if( ok )
        {
...
void FileReader::SetParams( OpenFileParams * params ) { m_params = params; }
                           ...
                           bool isLabel )
{
    if( QFile::exists( filename ) )
    {
        ...
        // temporary files cannot be removed before the end of the session - remove on exit from the application or on
        // load scene
        ...
        {
            readerTag->Delete();
            if( OpenTagFile( readObjects, filename, dataObjectName ) ) return true;
        }
        else
            readerTag->Delete();
        ...
        {
            readerObj->Delete();
            if( OpenObjFile( readObjects, filename, dataObjectName ) ) return true;
        }
        else
            readerObj->Delete();
        ...
        {
            if( OpenWavObjFile( readObjects, filename, dataObjectName ) ) return true;
        }

        // Try ply format
        if( filename.endsWith( QString( ".ply" ) ) )
        {
            if( OpenPlyFile( readObjects, filename, dataObjectName ) ) return true;
        }

        // try tractogram fib / vtk format
        if( OpenFIBFile( readObjects, filename, dataObjectName ) ) return true;

        // try vtp
        if( OpenVTPFile( readObjects, filename, dataObjectName ) ) return true;

        // finally try vtk
        if( OpenVTKFile( readObjects, filename, dataObjectName ) ) return true;
    }
    QString tmp( "File \"" );
    tmp.append( filename );
...
bool FileReader::OpenItkLabelFile( QList<SceneObject *> & readObjects, QString filename,
...
bool FileReader::OpenObjFile( QList<SceneObject *> & readObjects, QString filename, const QString & dataObjectName )
{
    ...
    m_fileProgressEvent->Connect( reader, vtkCommand::ProgressEvent, this,
                                  SLOT( OnReaderProgress( vtkObject *, unsigned long ) ), 0, 0.0,
                                  Qt::DirectConnection );
    reader->Update();
...
bool FileReader::OpenVTKFile( QList<SceneObject *> & readObjects, QString filename, const QString & dataObjectName )
{
    ...
    if( reader->GetErrorCode() == vtkErrorCode::NoError )
    {
        if( reader->IsFilePolyData() )
        {
            ...
            m_fileProgressEvent->Connect( polyReader, vtkCommand::ProgressEvent, this,
                                          SLOT( OnReaderProgress( vtkObject *, unsigned long ) ), 0, 0.0,
                                          Qt::DirectConnection );
            polyReader->Update();
        ...
        else if( reader->IsFileStructuredPoints() )
        {
            ...
            m_fileProgressEvent->Connect( pReader, vtkCommand::ProgressEvent, this,
                                          SLOT( OnReaderProgress( vtkObject *, unsigned long ) ), 0, 0.0,
                                          Qt::DirectConnection );
            pReader->Update();
...
void FileReader::ReportWarning( QString warning ) { m_warnings.push_back( warning ); }
...
bool FileReader::GetGrayFrame( QString filename, IbisItkUnsignedChar3ImageType::Pointer itkImage )
{
    ...
    itk::ImageRegionConstIterator<IbisItkUnsignedChar3ImageType> outputIterator(
        reader->GetOutput(), reader->GetOutput()->GetLargestPossibleRegion() );
    itk::ImageRegionIterator<IbisItkUnsignedChar3ImageType> inputIterator( itkImage,
...
bool FileReader::GetRGBFrame( QString filename, IbisRGBImageType::Pointer itkImage )
{
    ...
    itk::ImageRegionConstIterator<IbisRGBImageType> outputIterator( reader->GetOutput(),
