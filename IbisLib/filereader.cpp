...
void FileReader::SetIbisAPI( IbisAPI *api )
{
    ...
    if( m_ibisAPI )
    {
        QString  mincDir = m_ibisAPI->GetCustomVariable( MINCToolsPathVarName );
        if( mincDir != QString::null && !mincDir.isEmpty() )
        {
            ...
            if( !ok )
            {
                m_ibisAPI->UnRegisterCustomVariable( MINCToolsPathVarName );
                ok = FindMincConverter();
                if( ok )
                {
                    QFileInfo fi( m_mincconvert );
                    m_ibisAPI->RegisterCustomVariable( MINCToolsPathVarName, fi.absolutePath() );
                }
                else
                {
                    m_ibisAPI->RegisterCustomVariable( MINCToolsPathVarName, "" );
                    m_mincconvert.clear();
                    m_minccalc.clear();
                }
            }
