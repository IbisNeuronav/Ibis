...
void FileReader::run()
{
    ...
    for( int i = 0; i < m_params->filesParams.size(); ++i )
    {
        ...
        OpenFile( readObjects, param.fileName, param.objectName, param.isLabel, param.isVideoFrame , param.isMINC1 );
...
bool FileReader::OpenFile( QList<SceneObject*> & readObjects, QString filename, const QString & dataObjectName,
                           bool isLabel, bool  isVideoFrame, bool isMINC1)
{
    if( QFile::exists( filename ) )
    {
        ...
        if( isMINC1 )
        {
            ...
            {
                QString fileMINC2( QDir::homePath() );
                fileMINC2.append( "/.ibis/tmp/" );
                QDir tmpDir( fileMINC2 );
                bool ok = true;
                if( !tmpDir.exists() )
                    ok = tmpDir.mkdir(fileMINC2);
                if( ok )
                {
                    QFileInfo fi( filename );
                    fileMINC2.append( fi.fileName() );
                    //here I should maybe check if fileMINC2 = filename but, would user open files in MINC1 format from .ibis/tmp???
                    QString program( m_mincconvert );
                    QStringList arguments;
                    arguments << "-2" << filename << fileMINC2 << "-clobber" ;
                    QProcess *convertProcess = new QProcess(0);
                    convertProcess->start(program, arguments);
                    if( ok = convertProcess->waitForStarted())
                    ...
                    {
                        QString tmp( "Errors converting file " );
                        tmp.append( filename + "\nfrom MINC1 to MINC2 format" );
        ...
        {
            fileOpened = OpenItkVideoFrameFile(readObjects, fileToOpen, dataObjectName );
        }
        else
        {
...
//    itk::MetaDataDictionary &dictionary = reader->GetMetaDataDictionary();
//    vtkMINCImageAttributes2 *imageAttributes = vtkMINCImageAttributes2::New();
//    this->GetImageAttributes(dictionary, imageAttributes);
//    image->SetMINCImageAttributes(imageAttributes);
//    imageAttributes->Delete();
...
bool FileReader::OpenItkVideoFrameFile( QList<SceneObject*> & readObjects, QString filename, const QString & dataObjectName )
{
    // try to read
...
bool FileReader::OpenVTKFile( QList<SceneObject*> & readObjects, QString filename, const QString & dataObjectName )
{
    ...
    if( reader->GetErrorCode() == vtkErrorCode::NoError )
    {
        ...
        {
            ReportWarning( tr("Currently IBIS supports only DTI tracts.\nOther files may not be displayed correctly.") );
        }
        if (reader->IsFilePolyData())
        {
            ...
            if (isTracks)
            {
                DTIObject * object = DTIObject::New();
