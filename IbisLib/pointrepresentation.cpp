...
void PointRepresentation::Hide()
{
    ...
    while( it != m_perViewContainer.end() )
    {
        ...
        if( perView->labelActor ) perView->labelActor->VisibilityOff();
        ++it;
    }
}
...
void PointRepresentation::Show()
{
    ...
    while( it != m_perViewContainer.end() )
    {
        ...
        if( perView->labelActor ) perView->labelActor->SetVisibility( m_labelVisible ? 1 : 0 );
        ++it;
    }
}
...
void PointRepresentation::SetPointSizeIn2D( double r ) { m_circle->SetRadius( r ); }
...
bool PointRepresentation::HasActor( vtkActor * actor )
{
    ...
    {
        PerViewElements * perView = ( *it ).second;
        if( perView->pointRepresentationActor == actor ) return true;
        ++it;
    }
    return false;
}
...
{
    m_labelScale = s;
    if( m_labelScale <= 0 ) m_labelScale = 1.0;
    PerViewContainer::iterator it = m_perViewContainer.begin();
    while( it != m_perViewContainer.end() )
    {
        PerViewElements * perView = ( *it ).second;
        if( perView->labelActor ) perView->labelActor->SetScale( m_labelScale, m_labelScale, m_labelScale );
        ++it;
    }
}

void PointRepresentation::SetLabel( const std::string & text ) { m_label->SetText( text.c_str() ); }

const char * PointRepresentation::GetLabel() { return m_label->GetText(); }
...
void PointRepresentation::ShowLabel( bool show )
{
    ...
    {
        PerViewElements * perView = ( *it ).second;
        if( perView->labelActor ) perView->labelActor->SetVisibility( showNow ? 1 : 0 );
        ++it;
    }
}
