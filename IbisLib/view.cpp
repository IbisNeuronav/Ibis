...
{
    if( w == this->RenderWidget ) return;
    this->RenderWidget = w;
}

void View::SetRenderingEnabled( bool b )
{
    if( m_renderingEnabled == b ) return;
    m_renderingEnabled = b;
    if( m_renderingEnabled ) NotifyNeedRender();
}

vtkRenderWindowInteractor * View::GetInteractor() { return this->Interactor; }
...
void View::SetInteractor( vtkRenderWindowInteractor * interactor )
{
    ...
    this->EventObserver->Connect( this->Interactor->GetRenderWindow(), vtkCommand::StartEvent, this,
                                  SLOT( WindowStartsRendering() ) );
...
vtkRenderer * View::GetRenderer( int level )
{
    ...
    if( level == 0 ) return GetRenderer();
    if( level == 1 ) return GetOverlayRenderer();
    if( level == 2 ) return GetOverlayRenderer2();

    // Just in case
    return GetRenderer();
}

vtkRenderer * View::GetRenderer() { return this->Renderer; }

vtkRenderer * View::GetOverlayRenderer() { return this->OverlayRenderer; }

vtkRenderer * View::GetOverlayRenderer2() { return this->OverlayRenderer2; }
...
void View::SetInteractorStyle( vtkInteractorStyle * style )
{
    ...
    if( this->InteractorStyle )
    {
        if( this->Interactor ) this->Interactor->SetInteractorStyle( this->InteractorStyle );
    }
}

vtkInteractorStyle * View::GetInteractorStyle() { return this->InteractorStyle; }

void View::ReleaseView()
{
    if( CurrentController ) CurrentController->ReleaseControl( this );
    this->ReleaseAllObjects();
...
void View::ProcessInteractionEvents( vtkObject * /*caller*/, unsigned long event, void * /*calldata*/ )
{
    ...
    unsigned modifier = 0;
    if( this->Interactor->GetControlKey() ) modifier |= CtrlModifier;
    if( this->Interactor->GetShiftKey() ) modifier |= ShiftModifier;
    if( this->Interactor->GetAltKey() ) modifier |= AltModifier;
...
{
    if( this->GetType() != THREED_VIEW_TYPE ) this->Reset2DView();
...
double View::GetViewAngle() { return this->Renderer->GetActiveCamera()->GetViewAngle(); }
...
void View::EnableRendering() { this->SetRenderingEnabled( true ); }
...
void View::ReferenceTransformChanged()
{
    ...
    if( this->Manager && ( this->GetType() != THREED_VIEW_TYPE || this->Manager->Is3DViewFollowingReferenceVolume() ) )
    {
        ...
        if( obj && this->Renderer )
        {
            ...
            if( this->GetType() == THREED_VIEW_TYPE ) this->SetRotationCenter3D();
...
void View::WindowStartsRendering() { this->Renderer->ResetCameraClippingRange(); }
...
void View::ReleaseAllObjects() { this->Manager->ReleaseAllObjects( this ); }
