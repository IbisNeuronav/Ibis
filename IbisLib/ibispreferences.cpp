...
IbisPreferences::IbisPreferences( QObject * parent ) : QObject( parent ) { m_prefWidget = nullptr; }
...
void IbisPreferences::UnRegisterPath( const QString & pathName ) { m_customPaths.remove( pathName ); }
...
{
    QMap<QString, QString>::const_iterator it = m_customPaths.find( pathName );
    if( it != m_customPaths.end() ) return true;
    return false;
}
...
void IbisPreferences::OnPreferenceWidgetClosed() { m_prefWidget = nullptr; }
