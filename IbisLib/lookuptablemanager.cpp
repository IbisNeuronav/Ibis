...
void LookupTableManager::CreateLookupTable( const QString tableName, double range[2],
                                            vtkSmartPointer<vtkPiecewiseFunctionLookupTable> ct )
{
    ct->SetScaleToLinear();
    ct->SetRange( range );
    ct->RemoveAllPoints();
    ...
    else if( tableName == "Path Planning" )
    {
        ...
        ct->AddColorPoint( 0.25, 1.0, 1.0,
                           0.0 );  // yellow <-- I would really like to be able to move this point on the GUI)
...
int LookupTableManager::GetNumberOfTemplateLookupTables() const { return 13; }
...
void LookupTableManager::CreateLabelLookupTable( vtkSmartPointer<vtkLookupTable> lut )
{
    ...
    for( int i = 1; i < 256; ++i ) lut->SetTableValue( i, labelColors[i][0], labelColors[i][1], labelColors[i][2] );
}
