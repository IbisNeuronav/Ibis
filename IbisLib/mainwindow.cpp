      ...
      m_windowClosing( false )
{
    ...
    m_viewXPlaneAction = AddToggleAction( viewMenu, tr( "&Sagittal Plane" ), SLOT( ViewXPlaneToggled( bool ) ),
                                          QKeySequence( "Alt+x" ), true );
    m_viewYPlaneAction = AddToggleAction( viewMenu, tr( "&Coronal Plane" ), SLOT( ViewYPlaneToggled( bool ) ),
                                          QKeySequence( "Alt+y" ), true );
    ...
    connect( objectListWidget, SIGNAL( ObjectSettingsWidgetChanged( QWidget * ) ), this,
             SLOT( ObjectListWidgetChanged( QWidget * ) ) );
    ...
    if( validIcon ) setWindowIcon( icon );
...
void MainWindow::CreatePluginsUi()
{
    ...
    connect( &( Application::GetInstance() ), SIGNAL( QueryActivatePluginSignal( ToolPluginInterface *, bool ) ), this,
             SLOT( ToggleToolPlugin( ToolPluginInterface *, bool ) ) );
}
...
void MainWindow::AboutPlugins()
{
    ...
    w->setWindowFlags( Qt::WindowStaysOnTopHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint |
...
void MainWindow::OpenFiles( OpenFileParams * params ) { Application::GetInstance().OpenFiles( params ); }
...
void MainWindow::fileImportUsAcquisition() { Application::GetInstance().ImportUsAcquisition(); }

void MainWindow::fileImportCamera() { Application::GetInstance().ImportCamera(); }
...
void MainWindow::fileSaveScene() { this->SaveScene( false ); }

void MainWindow::fileSaveSceneAs() { this->SaveScene( true ); }
...
void MainWindow::SaveScene( bool asFile )
{
    ...
    if( asFile || sceneDir.isEmpty() )
    {
        ...
        fileName =
            Application::GetInstance().GetFileNameSave( tr( "Save Scene" ), initialFile, tr( "xml file (*.xml)" ) );
        if( !fileName.isEmpty() )
        {
...
void MainWindow::fileLoadScene()
{
    ...
    QString fileName =
        Application::GetInstance().GetFileNameOpen( tr( "Load Scene" ), workingDir, tr( "Scene files (*.xml)" ) );
...
void MainWindow::ToolPluginsMenuActionToggled( bool isOn )
{
    ...
    if( isOn )
    {
        ...
        else
        {
            ...
            {
                pluginWidget->setAttribute( Qt::WA_DeleteOnClose );
                pluginWidget->setWindowFlags( Qt::WindowStaysOnTopHint | Qt::WindowTitleHint |
...
void MainWindow::FloatingPluginWidgetClosed()
{
    ...
    if( !toolPlugin->WidgetAboutToClose() ) return;
    ...
    if( !m_windowClosing ) toolPlugin->GetSettings().active = false;
...
void MainWindow::PluginTabClosed( int tabIndex )
{
    ...
    Q_ASSERT_X( pluginAction, "MainWindow::PluginTabClosed( int tabIndex )",
                "No action associated with closed widget." );
...
void MainWindow::ClosePluginTab( QAction * action, int index )
{
    ...
    if( m_pluginTabs.size() == 0 ) UpdateMainSplitter();
...
void MainWindow::ObjectPluginsMenuActionTriggered()
{
    ...
    if( obj ) Application::GetSceneManager()->SetCurrentObject( obj );
}
...
void MainWindow::closeEvent( QCloseEvent * event )
{
    ...
    {
        if( toolPlugin->IsPluginActive() ) toolPlugin->WidgetAboutToClose();
    }
...
bool MainWindow::eventFilter( QObject * obj, QEvent * event )
{
    ...
    if( !handled ) return QObject::eventFilter( obj, event );

    return true;
}

void MainWindow::AddBottomWidget( QWidget * w ) { m_4Views->AddBottomWidget( w ); }

void MainWindow::RemoveBottomWidget( QWidget * w ) { m_4Views->RemoveBottomWidget( w ); }
...
void MainWindow::SetShowToolbar( bool show ) { m_4Views->SetShowToolbar( show ); }

void MainWindow::SetShowLeftPanel( bool show ) { m_leftFrame->setHidden( !show ); }

void MainWindow::SetShowRightPanel( bool show ) { m_rightPanel->setHidden( !show ); }

void MainWindow::Serialize( Serializer * ser ) { m_4Views->Serialize( ser ); }
...
void MainWindow::Preferences() { Application::GetInstance().Preferences(); }
