...
WorldObjectSettingsWidget::~WorldObjectSettingsWidget() { delete ui; }

void WorldObjectSettingsWidget::SetWorldObject( WorldObject * obj )
{
    if( obj == m_worldObject ) return;
...
{
    if( m_worldObject ) m_worldObject->SetAxesHidden( !checked );
    UpdateUi();
}

void WorldObjectSettingsWidget::on_showCursorCheckBox_toggled( bool checked )
{
    if( m_worldObject ) m_worldObject->SetCursorVisible( checked );
    UpdateUi();
}
