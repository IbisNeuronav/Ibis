...
void ObjectTreeWidget::SetSceneManager( SceneManager * man )
{
    ...
    if( m_sceneManager )
    {
        ...
        connect( ui->treeView->selectionModel(), SIGNAL( selectionChanged( QItemSelection, QItemSelection ) ), this,
                 SLOT( SelectionChanged( QItemSelection, QItemSelection ) ) );
        connect( ui->treeView->model(), SIGNAL( dataChanged( QModelIndex, QModelIndex ) ), this,
        ...
        connect( m_sceneManager, SIGNAL( StartAddingObject( SceneObject *, int ) ), this,
                 SLOT( AddingObject( SceneObject *, int ) ) );
    }
    else
...
{
    if( m_workingOnSelection ) return;
...
void ObjectTreeWidget::CurrentObjectChanged()
{
    ...
    if( currentObject )
    {
        ...
        if( ind.isValid() ) sel.select( ind, ind );
    }
    ui->treeView->selectionModel()->select( sel, QItemSelectionModel::ClearAndSelect );
    ...
    if( object )
    {
        ...
        if( settingsWidget )
        {
            ...
            if( !lastOpenTabLabel.isEmpty() )
            {
                ...
                if( notFound ) settingsWidget->setCurrentIndex( 0 );
            }
            else
...
void ObjectTreeWidget::contextMenuEvent( QContextMenuEvent * event )
{
    ...
    if( currentObj )
    {
        ...
        if( currentObj->IsObjectDeletable() || currentObj->IsExportable() ||
            m_sceneManager->CanBeReference( currentObj ) )
        {
            QMenu contextMenu;
            contextMenu.addAction( tr( "" ) );
            if( currentObj->IsExportable() ) contextMenu.addAction( tr( "Export" ), this, SLOT( ExportObject() ) );
            ...
            if( currentObj->IsA( "ImageObject" ) )
            {
                ...
                if( obj->GetItkImage() ) contextMenu.addAction( tr( "Show MINC Info" ), this, SLOT( ShowMincInfo() ) );
            }
...
{
    SceneObject * obj = m_sceneManager->GetCurrentObject();
    if( obj && obj->IsExportable() ) obj->Export();
}

void ObjectTreeWidget::HideWithChildren()
{
    SceneObject * obj = m_sceneManager->GetCurrentObject();
    if( obj && obj->IsHidable() ) obj->SetHiddenWithChildren( obj->IsHidden() ? false : true );
}
