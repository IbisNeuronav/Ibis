...
const char shaderContributionDiffuseShade[] = "vec4 sample = SampleVolumeWithTransferFunction( volIndex, pos ); \n\
fullSample += sample; \n\
if( sample.a > 0.1 ) \n\
{ \n\
    float gradStep = 0.01; \n\
...
if( dplane > 0.0 ) \n\
{	\n\
    fogStartDist = dplane + 0.1; \n\
    fogEndDist = fogStartDist + 0.2; \n\
    if( fogEndDist < stopDist ) \n\
        stopDist = fogEndDist; \n\
} \n\
else \n\
    stopDist = 0.0;";
...
VolumeRenderingObject::VolumeRenderingObject()
{
    m_isAnimating = false;
    m_multFactor = 1.0;
    m_fogMinDistance = 100.0;
    m_fogMaxDistance = 1000.0;
    m_fogEnabled = false;
...
void VolumeRenderingObject::Serialize( Serializer * ser )
{
    ...
    ser->Serialize( "FogEnabled", m_fogEnabled );
    ser->Serialize( "FogMinDistance", m_fogMinDistance );
    ser->Serialize( "FogMaxDistance", m_fogMaxDistance );
...
bool VolumeRenderingObject::Setup( View * view )
{
    if( view->GetType() == THREED_VIEW_TYPE )
    {
        ...
        perView.volumeMapper->SetFogEnabled( m_fogEnabled );
        perView.volumeMapper->SetMinFogDistance( m_fogMinDistance );
        perView.volumeMapper->SetMaxFogDistance( m_fogMaxDistance );
        ...
        //perView.sphereWidget->SetRadius( 5.0 );
        //perView.sphereWidget->SetRepresentationToSurface();
        //perView.sphereWidget->GetSphereProperty()->SetColor( 1.0, 0.85, 0.0 );
...
QWidget * VolumeRenderingObject::CreateSettingsDialog( QWidget * parent )
{
    ...
    widget->SetSceneManager( this->GetManager() );
    return widget;
}
...
void VolumeRenderingObject::SetMultFactor( double val )
{
 ...
 void VolumeRenderingObject::SetFogEnabled( bool on )
 {
     m_fogEnabled = on;
     UpdateShaderParams();
     emit Modified();
...
void VolumeRenderingObject::SetFogMinDistance( double dist )
{
    m_fogMinDistance = dist;
    UpdateShaderParams();
    emit Modified();
}

void VolumeRenderingObject::SetFogMaxDistance( double dist )
{
    m_fogMaxDistance = dist;
    UpdateShaderParams();
    emit Modified();
}
...
void VolumeRenderingObject::TrackingUpdatedSlot()
{
    ...
    if( navPointer )
    {
        ...
        double * axisPoint = navPointer->GetMainAxisPosition();
        m_interactionPoint2[0] = axisPoint[0];
        m_interactionPoint2[1] = axisPoint[1];
        m_interactionPoint2[2] = axisPoint[2];
...
void VolumeRenderingObject::UpdateMapper( PerView & pv )
{
    ...
    mapper->SetFogEnabled( m_fogEnabled );
    mapper->SetMinFogDistance( m_fogMinDistance );
    mapper->SetMaxFogDistance( m_fogMaxDistance );
