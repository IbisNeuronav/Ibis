...
CameraCalibrationPluginInterface::CameraCalibrationPluginInterface()
{
    ...
    m_calibrationGridWidth =
        6;  // This is the number of crossings being considered for calibration and not the number of squares
...
CameraCalibrationPluginInterface::~CameraCalibrationPluginInterface()
{
    if( m_calibrationGridObject ) m_calibrationGridObject->Delete();
...
{
    return QString(
        "<html><h1>Camera Calibration plugin</h1><p>Author: Simon Drouin</p><p>Description</p><p>This plugin is used "
        "to calibrate a tracked camera to produce an augmented reality image</p></html>" );
}

bool CameraCalibrationPluginInterface::CanRun() { return true; }
...
{
    if( IsCalibrationWidgetOn() ) m_cameraCalibrationWidget->close();
...
bool CameraCalibrationPluginInterface::IsCalibrationWidgetOn() { return ( m_cameraCalibrationWidget != 0 ); }
...
void CameraCalibrationPluginInterface::OnObjectAddedOrRemoved() { this->ValidateCurrentCamera(); }
...
void CameraCalibrationPluginInterface::GetAllValidCamerasFromScene( QList<CameraObject *> & all )
{
    ...
    {
        if( !needTrackableCamera || allCams[i]->IsDrivenByHardware() ) all.push_back( allCams[i] );
    }
}
...
void CameraCalibrationPluginInterface::SetComputeIntrinsic( bool c ) { m_computeIntrinsic = c; }

void CameraCalibrationPluginInterface::SetComputeExtrinsic( bool c ) { m_computeExtrinsic = c; }

void CameraCalibrationPluginInterface::SetExtrinsicTranslationScale( double s ) { m_extrinsicTranslationScale = s; }

void CameraCalibrationPluginInterface::SetExtrinsicRotationScale( double s ) { m_extrinsicRotationScale = s; }
...
void CameraCalibrationPluginInterface::DoCalibration()
{
    ...
    if( currentCamera && m_cameraCalibrator->GetNumberOfViews() > 0 )
    {
        ...
        if( m_computeExtrinsic )
        {
            ...
            if( gridTransform ) gridTransform->SetMatrix( extParams.gridMatrix );
        }
        UpdateCameraViewsObjects();
    ...
    if( currentCam )
    {
        ...
        for( int i = 0; i < nbViews; ++i )
        {
            if( m_cameraCalibrator->IsViewEnabled( i ) )
            {
                ...
                calibMat->Delete();
                newCam->AddFrame( m_cameraCalibrator->GetCameraImage( i ),
                                  m_cameraCalibrator->GetIntrinsicCameraToGridMatrix( i ) );
...
void CameraCalibrationPluginInterface::ValidateCurrentCamera()
{
    ...
    if( m_currentCameraObjectId != IbisAPI::InvalidId )
    {
        ...
        if( !camObj || ( needTrackableCamera && !camObj->IsDrivenByHardware() ) ) newCameraId = IbisAPI::InvalidId;
    }
...
void CameraCalibrationPluginInterface::ClearCameraViews()
{
    ...
    for( unsigned i = 0; i < m_cameraViewsObjectIds.size(); ++i )
    {
        ...
        if( cam ) GetIbisAPI()->RemoveObject( cam );
    }
