...
void ComputeReprojection( vector<Point2f> & imagePoints, vector<Point3f> & worldPoints,
                          vector<Point2f> & projectedPoints, vector<double> & reprojDistmm,
                          vector<double> & pointDistmm, int imageSize[2], double focal[2], double center[2],
...
bool CameraCalibrator::GetOptimizeGridDetection() { return m_optimizeGridDetection; }

void CameraCalibrator::SetOptimizeGridDetection( bool optimize ) { m_optimizeGridDetection = optimize; }
...
void CameraCalibrator::ExportCalibrationData( QString dir, QProgressDialog * progressDlg )
{
    ...
    for( int i = 0; i < GetNumberOfViews(); ++i )
    {
        ...
        QString calibrationPointsFileName =
            dir + QString( "/calibrationPoints_%1.txt" ).arg( i, 4, 10, QLatin1Char( '0' ) );
        QFile calibrationPointsFile( calibrationPointsFileName );
        if( !calibrationPointsFile.open( QIODevice::WriteOnly | QIODevice::Text ) ) return;
        QTextStream pointsOut( &calibrationPointsFile );
        pointsOut << m_imagePoints[i].size() << " " << GetGridWidth() << " " << GetGridHeight() << " "
                  << GetGridCellSize() << "\n";
        ...
        if( progressDlg )
            m_pluginInterface->GetIbisAPI()->UpdateProgress( progressDlg,
                                                             (int)round( (float)i / m_cameraImages.size() * 100.0 ) );
    }
    writer->Delete();
}
...
void CameraCalibrator::ImportCalibrationData( QString dirName, QProgressDialog * progressDlg )
{
    ...
    {
        QString calibrationPointsFileName =
            dirName + QString( "/calibrationPoints_%1.txt" ).arg( numberOfViews, 4, 10, QLatin1Char( '0' ) );
        if( !ExistsAndReadable( calibrationPointsFileName ) ) break;

        QString trackerMatrixFileName =
            dirName + QString( "/trackerMatrix_%1.txt" ).arg( numberOfViews, 4, 10, QLatin1Char( '0' ) );
        if( !ExistsAndReadable( trackerMatrixFileName ) ) break;

        QString imageFileName = dirName + QString( "/frame_%1" ).arg( numberOfViews, 4, 10, QLatin1Char( '0' ) );
        if( !ExistsAndReadable( imageFileName ) ) break;
    ...
    for( int i = 0; i < numberOfViews; ++i )
    {
        ...
        QString calibrationPointsFileName =
            dirName + QString( "/calibrationPoints_%1.txt" ).arg( i, 4, 10, QLatin1Char( '0' ) );
        ...
        if( i == 0 ) SetGridProperties( gridWidth, gridHeight, gridSpacing );
        ...
        QString trackerMatrixFileName =
            dirName + QString( "/trackerMatrix_%1.txt" ).arg( i, 4, 10, QLatin1Char( '0' ) );
        ...
        if( progressDlg )
            m_pluginInterface->GetIbisAPI()->UpdateProgress( progressDlg,
                                                             (int)round( (float)i / numberOfViews * 100.0 ) );
    }
    reader->Delete();
}
...
bool CameraCalibrator::DetectGrid( vtkImageData * im, std::vector<cv::Point2f> & imagePoints )
{
    ...
    bool found = cv::findChessboardCorners(
        imageSmall, patternSize, pointsSmall,
    ...
    if( found )
    {
        ...
        cv::cornerSubPix( imageGray, imagePoints, cv::Size( 11, 11 ), cv::Size( -1, -1 ),
...
void CameraCalibrator::AddView( vtkImageData * im, std::vector<cv::Point2f> & imagePoints,
...
void CameraCalibrator::Calibrate( bool computeCenter, bool computeDistortion, CameraIntrinsicParams & params,
                                  CameraExtrinsicParams & extParams )
{
    ...
    // Do the calibration - we always ignore tangential distortion and higher-order terms in the radial distortion.
    // There is an option to allow moving the center point and optimizing for the first radial distortion coefficient
    int flags = CV_CALIB_ZERO_TANGENT_DIST | CV_CALIB_FIX_K2 | CV_CALIB_FIX_K3 | CV_CALIB_FIX_K4 | CV_CALIB_FIX_K5 |
                CV_CALIB_FIX_K6;
    if( !computeCenter ) flags |= CV_CALIB_FIX_PRINCIPAL_POINT;
    if( !computeDistortion ) flags |= CV_CALIB_FIX_K1;
    params.m_reprojectionError =
...
void CameraCalibrator::ComputeMeanAndStdDevOfGridCorners( cv::Point3f & meanTL, cv::Point3f & meanTR,
                                                          cv::Point3f & meanBL, cv::Point3f & meanBR, double & val )
{
    ...
    // Compute
    double valTopLeft =
        sqrt( ( sstl[0] - stl[0] * stl[0] / n ) / ( n - 1 ) + ( sstl[1] - stl[1] * stl[1] / n ) / ( n - 1 ) +
              ( sstl[2] - stl[2] * stl[2] / n ) / ( n - 1 ) );
    double valTopRight =
        sqrt( ( sstr[0] - str[0] * str[0] / n ) / ( n - 1 ) + ( sstr[1] - str[1] * str[1] / n ) / ( n - 1 ) +
              ( sstr[2] - str[2] * str[2] / n ) / ( n - 1 ) );
    double valbottomLeft =
        sqrt( ( ssbl[0] - sbl[0] * sbl[0] / n ) / ( n - 1 ) + ( ssbl[1] - sbl[1] * sbl[1] / n ) / ( n - 1 ) +
              ( ssbl[2] - sbl[2] * sbl[2] / n ) / ( n - 1 ) );
    double valbottomRight =
        sqrt( ( ssbr[0] - sbr[0] * sbr[0] / n ) / ( n - 1 ) + ( ssbr[1] - sbr[1] * sbr[1] / n ) / ( n - 1 ) +
              ( ssbr[2] - sbr[2] * sbr[2] / n ) / ( n - 1 ) );
...
double CameraCalibrator::ComputeCrossValidation( double translationScale, double rotationScale,
                                                 double & stdDevReprojError, double & minDist, double & maxDist,
                                                 QProgressDialog * progressDlg )
{
    ...
    for( int xvalIndex = 0; xvalIndex < numberOfViews; ++xvalIndex )
    {
        if( m_viewEnabled[xvalIndex] )
        {
            ...
            ComputeReprojection( m_imagePoints[xvalIndex], m_objectPoints[xvalIndex], projectedPoints, reprojDistmm,
                                 pointDistmm, imageSize, intrinsicParams.m_focal, intrinsicParams.m_center,
                                 intrinsicParams.m_distorsionK1, m_trackingMatrices[xvalIndex], extrinsicParams );

            allReprojErrors.insert( allReprojErrors.end(), reprojDistmm.begin(), reprojDistmm.end() );
            for( int k = 0; k < reprojDistmm.size(); ++k )
            {
                if( pointDistmm[k] > maxDist ) maxDist = pointDistmm[k];
                if( pointDistmm[k] < minDist ) minDist = pointDistmm[k];
            }
        ...
        if( progressDlg )
            m_pluginInterface->GetIbisAPI()->UpdateProgress( progressDlg,
                                                             (int)round( (float)xvalIndex / numberOfViews * 100.0 ) );
    }

    // Compute mean reprojection error
    int nbPoints            = nbEnabledViews * m_gridWidth * m_gridHeight;
    double sumReprojErrormm = 0.0;
    for( int i = 0; i < allReprojErrors.size(); ++i ) sumReprojErrormm += allReprojErrors[i];
...
double CameraCalibrator::ComputeAvgReprojectionError( CameraIntrinsicParams & intParams,
                                                      CameraExtrinsicParams & extParams )
{
    double sumReprojErrormm = 0.0;
    ...
    for( int v = 0; v < GetNumberOfViews(); ++v )
    {
        if( m_viewEnabled[v] )
        {
            ...
            imageSize[0] = GetCameraImageWidth();
            imageSize[1] = GetCameraImageHeight();
            ComputeReprojection( m_imagePoints[v], m_objectPoints[v], projectedPoints, reprojDistmm, pointDistmm,
                                 imageSize, intParams.m_focal, intParams.m_center, intParams.m_distorsionK1,
                                 m_trackingMatrices[v], extParams );
...
void CameraCalibrator::SetPluginInterface( CameraCalibrationPluginInterface * ifc ) { m_pluginInterface = ifc; }
