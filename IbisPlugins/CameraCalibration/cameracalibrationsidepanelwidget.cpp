...
CameraCalibrationSidePanelWidget::CameraCalibrationSidePanelWidget( QWidget * parent )
    : QWidget( parent ), ui( new Ui::CameraCalibrationSidePanelWidget )
{
    ui->setupUi( this );
    m_pluginInterface = 0;
}

CameraCalibrationSidePanelWidget::~CameraCalibrationSidePanelWidget() { delete ui; }
...
void CameraCalibrationSidePanelWidget::InterfaceModified() { UpdateUi(); }
...
void CameraCalibrationSidePanelWidget::UpdateUi()
{
    ...
    if( allCams.size() == 0 ) ui->currentCameraComboBox->addItem( "None", QVariant( IbisAPI::InvalidId ) );
    ...
    ui->extrinsicReprojErrorLabel->setText(
        QString( "Avg. extrinsic reproj err.: %1" ).arg( m_pluginInterface->GetMeanRoprojectionError() ) );
...
void CameraCalibrationSidePanelWidget::on_exportCalibrationDataButton_clicked()
{
    ...
    QString dir = QFileDialog::getExistingDirectory( this, "Choose directory to export calibration data",
                                                     QDir::homePath(), QFileDialog::DontUseNativeDialog );
    if( !dir.isEmpty() )
    {
...
void CameraCalibrationSidePanelWidget::on_importDataButton_clicked()
{
    ...
    QString dir = QFileDialog::getExistingDirectory( this, "Choose directory where calibration data is located",
                                                     QDir::homePath(), QFileDialog::DontUseNativeDialog );
    if( !dir.isEmpty() )
    {
...
void CameraCalibrationSidePanelWidget::on_validationButton_clicked()
{
    ...
    double minDist           = 0.0;
    double maxDist           = 0.0;
    double avgReprojError    = m_pluginInterface->GetCameraCalibrator()->ComputeCrossValidation(
        tScale, rScale, stdDevReprojError, minDist, maxDist, dlg );
    m_pluginInterface->GetIbisAPI()->StopProgress( dlg );

    QString validationText =
        QString( "Average reprojection error: %1 +/- %2 mm\n" ).arg( avgReprojError ).arg( stdDevReprojError );
...
void CameraCalibrationSidePanelWidget::on_updateCalibrationButton_clicked() { m_pluginInterface->DoCalibration(); }
