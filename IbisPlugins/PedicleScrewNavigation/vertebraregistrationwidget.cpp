...
{
    if( !m_isNavigating || !m_navigationWidget ) return nullptr;
...
void VertebraRegistrationWidget::GetUSScanOrthogonalDirection(
    itk::Vector<double, 3> & directionVector, std::vector<itk::SmartPointer<IbisItkFloat3ImageType> > inputImageList,
    std::vector<itk::Point<double, 3> > usScanCenterPointList, vtkTransform * parentTransform )
{
    ...
    for( int i = 0; i < inputImageList.size(); ++i )
    {
        ...
        {
            float * temp;
            temp           = parentTransform->TransformFloatPoint( (float)bottomPoint[0], (float)bottomPoint[1],
                                                         (float)bottomPoint[2] );
            bottomPoint[0] = temp[0];
            bottomPoint[1] = temp[1];
            bottomPoint[2] = temp[2];

            temp           = parentTransform->TransformFloatPoint( (float)centerPoint[0], (float)centerPoint[1],
                                                         (float)centerPoint[2] );
...
void VertebraRegistrationWidget::PerformInitialAlignment(
    vtkTransform *& rigidTransform, IbisItkFloat3ImagePointer itkCTImage,
    std::vector<itk::SmartPointer<IbisItkFloat3ImageType> > inputImageList,
    std::vector<itk::Point<double, 3> > usScanCenterPointList, vtkTransform * parentUSAcquisitionTransform )
{
...

    {
        ...
        this->GetUSScanOrthogonalDirection( usOrthogonalDirection, inputImageList, usScanCenterPointList,
                                            parentUSAcquisitionTransform );
...
bool VertebraRegistrationWidget::Register()
{
    ...
    if( ctImageObjectId != SceneManager::InvalidId )
    {
        ...
        Q_ASSERT_X( ctImageObject, "VertebraRegistrationWidget::on_startRegistrationButton_clicked()",
                    "Invalid CT Image Object" );
        sourceVtkTransform = ctImageObject->GetLocalTransform();
        Q_ASSERT_X( sourceVtkTransform, "VertebraRegistrationWidget::on_startRegistrationButton_clicked()",
                    "Invalid CT Image Transform Object" );
    }
    else
    {
    ...
    if( usAcquisitionObjectId != SceneManager::InvalidId )
    {
        ...
        Q_ASSERT_X( usAcquisitionObject, "VertebraRegistrationWidget::on_startRegistrationButton_clicked()",
                    "Invalid US Acquisition Object" );
    }
    else
    {
    ...
    if( !this->CreateVolumeFromSlices( usAcquisitionObject, m_reconstructionResolution, secAcqList ) ) return false;
    ...
    Q_ASSERT_X( targetImageObject, "VertebraRegistrationWidget::on_startRegistrationButton_clicked()",
    ...
    if( ui->gradientAlignmentCheckBox->isChecked() )
    {
        ...
        if( ctImageObject->GetParent() )
        {
            vtkTransform * parentVtktransform =
                vtkTransform::SafeDownCast( ctImageObject->GetParent()->GetWorldTransform() );
            Q_ASSERT_X( parentVtktransform, "VertebraRegistrationWidget::on_startRegistrationButton_clicked()",
                        "Invalid transform" );
...
void VertebraRegistrationWidget::StartNavigation()
{
    if( !m_navigationWidget )
    {
        ...
        QFlags<QDockWidget::DockWidgetFeature> flags =
...
void VertebraRegistrationWidget::on_startRegistrationButton_clicked()
{
    if( !m_isProcessing )
    {
        ...
        if( processOK ) ui->elapsedTimeLabel->setText( tr( "Time: " ) + QString::number( elapsedTime ) + tr( " s" ) );
        m_isProcessing = false;
    }
}
...
void VertebraRegistrationWidget::on_navigateButton_clicked()
{
    if( !m_isNavigating )
    {
        ...
        {
            QMessageBox::information( this, "Pedicle Screw Navigation",
                                      "It seems that the plugin is unable to find a navigation pointer. Make sure that "
                                      "IGSIO is running properly." );
        }
    }
    else
    {
...
void VertebraRegistrationWidget::on_presetVolumeButton_clicked()
{
    if( m_pluginInterface )
    {
        ...
        if( objId == IbisAPI::InvalidId ) return;
...
void VertebraRegistrationWidget::on_addUSAcquisitionButton_clicked() { m_secondaryAcquisitions->addNewEntry(); }

void VertebraRegistrationWidget::on_removeUSAcquisitionButton_clicked() { m_secondaryAcquisitions->removeLastEntry(); }
