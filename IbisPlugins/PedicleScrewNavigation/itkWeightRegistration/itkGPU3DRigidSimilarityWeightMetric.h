...
public:
    ...
    void SetIntensityMetric( double value ) { m_CurrentIntensityMetricValue = value; }

    void SetGradientMetric( double value ) { m_CurrentGradientMetricValue = value; }
...
public:
    ...
    enum RegistrationMetricToUseType
    {
        INTENSITY   = 0,
        GRADIENT    = 1,
        COMBINATION = 2
    ...
    typedef itk::GPUWeightMatchingMatrixTransformationSparseMask<TFixedImage, TMovingImage> GPUIntensityMetricType;
    ...
    {
        if( !m_GPUIntensityMetric ) itkExceptionMacro( << "GPUIntensityMetric has not been set!" );

        if( !m_GPUOrientationMetric ) itkExceptionMacro( << "GPUOrientationMetric has not been set!" );
        ...
        if( ( m_RegistrationMetricToUse == GRADIENT ) | ( m_RegistrationMetricToUse == COMBINATION ) )
        {
            ...
            CurrentGradientMetricValue =
                (double)std::pow( m_GPUOrientationMetric->GetMetricValue(), 1.0 / m_OrientationSelectivity );
    ...
    {
        PointType temp;
        temp[0] = m_FixedImage->GetOrigin()[0] +
                  m_FixedImage->GetSpacing()[0] * m_FixedImage->GetBufferedRegion().GetSize()[0] / 2.0;
        temp[1] = m_FixedImage->GetOrigin()[1] +
                  m_FixedImage->GetSpacing()[1] * m_FixedImage->GetBufferedRegion().GetSize()[1] / 2.0;
        temp[2] = m_FixedImage->GetOrigin()[2] +
                  m_FixedImage->GetSpacing()[2] * m_FixedImage->GetBufferedRegion().GetSize()[2] / 2.0;
    ...
    unsigned int GetNumberOfParameters( void ) const override { return SpaceDimension; }

    double GetCurrentIntensityMetricValue() const { return m_MetricMonitor->GetIntesityMetric(); }

    double GetCurrentGradientMetricValue() const { return m_MetricMonitor->GetGradientMetric(); }

    void UpdateMetrics()
    {
        if( !m_FixedImage ) itkExceptionMacro( << "Fixed Image has not been set!" );

        if( !m_MovingImage ) itkExceptionMacro( << "Moving Image has not been set!" );

        if( !m_MetricTransform ) itkExceptionMacro( << "Metric Transform has not been set!" );

        if( !m_GPUIntensityMetric ) m_GPUIntensityMetric = GPUIntensityMetricType::New();

        if( !m_GPUOrientationMetric ) m_GPUOrientationMetric = GPUOrientationMetricType::New();
