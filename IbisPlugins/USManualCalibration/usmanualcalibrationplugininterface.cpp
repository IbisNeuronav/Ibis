...
bool USManualCalibrationPluginInterface::CanRun() { return true; }
...
QWidget * USManualCalibrationPluginInterface::CreateFloatingWidget()
{
    ...
    if( m_landmarkRegistrationObjectId == IbisAPI::InvalidId ) this->StartPhantomRegistration();
...
bool USManualCalibrationPluginInterface::WidgetAboutToClose()
{
    if( m_landmarkRegistrationObjectId != IbisAPI::InvalidId )
    {
        ...
        if( phantom ) this->GetIbisAPI()->RemoveObject( phantom );
        SceneObject * model = this->GetIbisAPI()->GetObjectByID( m_calibrationPhantomObjectId );
        if( model ) this->GetIbisAPI()->RemoveObject( model );
...
void USManualCalibrationPluginInterface::ValidateUsProbe()
{
    ...
    if( !probe ) m_usProbeObjectId = IbisAPI::InvalidId;
    if( m_usProbeObjectId == IbisAPI::InvalidId )
    {
        QList<UsProbeObject *> allProbes;
        GetIbisAPI()->GetAllUsProbeObjects( allProbes );
        if( allProbes.size() > 0 ) m_usProbeObjectId = allProbes[0]->GetObjectID();
    }
}
...
void USManualCalibrationPluginInterface::BuildWiresRepresentation()
{
    ...
    if( !needNewRepresentation ) return;
...
{
    bool needNewRepresentation = m_phantomWiresObjectId == IbisAPI::InvalidId;
    PolyDataObject * phantomObject =
        PolyDataObject::SafeDownCast( GetIbisAPI()->GetObjectByID( m_phantomWiresObjectId ) );
...
void USManualCalibrationPluginInterface::StartPhantomRegistration()
{
    ...
    if( m_calibrationPhantomObjectId == IbisAPI::InvalidId ) BuildCalibrationPhantomRepresentation();
    SceneObject * phantomObject = GetIbisAPI()->GetObjectByID( m_calibrationPhantomObjectId );

    // Add source and target points to scene
    PointsObject * sourcePoints =
        PointsObject::SafeDownCast( GetIbisAPI()->GetObjectByID( m_phantomRegSourcePointsId ) );
    ...
    if( !sourcePoints )
    {
        ...
        for( int i = 0; i < 4; ++i ) sourcePoints->AddPoint( pointNames[i], pointCoords[i] );
    }

    PointsObject * targetPoints =
        PointsObject::SafeDownCast( GetIbisAPI()->GetObjectByID( m_phantomRegTargetPointsId ) );
    ...
    if( !targetPoints )
    {
        ...
        for( int i = 0; i < 4; ++i ) targetPoints->AddPoint( pointNames[i], pointCoords[i] );
    }
    ...
    LandmarkRegistrationObjectPluginInterface * landmarkRegPlugin =
        LandmarkRegistrationObjectPluginInterface::SafeDownCast( basePlugin );
    ...
    // cleanup
    if( allocSource ) sourcePoints->Delete();
    if( allocTarget ) targetPoints->Delete();
}
