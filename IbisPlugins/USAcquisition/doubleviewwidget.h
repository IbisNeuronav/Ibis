 ...
 *
 *  DoubleViewWidget consists of two fixed size windows, placed horizontally side by side and surrounded by control
 * buttons. The left window is used to show the acquired frames, the right window shows some image data, usually
 * pre-operational MRI. Both images can be blended in the right side window. A second image may be added to the right
 * window and blended with the first one. DoubleViewWidget may be used to show a live acquisition or a previously
 * acquired and stored acquisition. Initially the class was developed for fixed size frames, changes must be made to
 * accomodate different size frames.
 *
...
public:
     ...
     * elements to show in the double view. */
...
protected:
    ...
    /** When capturing live frames, the probe may momentarily become invisible, red cross will then show in both windows
     * and frames won't be stored in the buffer as they are not valid. */
...
private slots:
    ...
    /** The right window will show blended right side and left side images, if there is a second image in the right
     * window it will also be blended in.*/
    ...
    /** Set the acquisition selected by index in the left window. The right window images will be adjusted
     * accordingly.*/
    ...
    /** If there is a US probe active, checking Live wil set the plugin in the acquisition mode. Left window will show
     * currently acquired frames, right window will show respective reslie.*/
