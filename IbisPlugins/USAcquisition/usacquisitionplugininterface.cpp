...
bool USAcquisitionPluginInterface::CanRun() { return true; }
...
void USAcquisitionPluginInterface::ValidateCurrentAcquisition()
{
    ...
    if( m_currentAcquisitionObjectId == SceneManager::InvalidId )
    {
        ...
        if( acquisitions.size() > 0 ) m_currentAcquisitionObjectId = acquisitions[0]->GetObjectID();
    }

    if( initialAcqId != m_currentAcquisitionObjectId ) emit ObjectsChanged();
}
...
void USAcquisitionPluginInterface::ValidateCurrentUsProbe()
{
    ...
    IbisAPI * ibisAPI  = GetIbisAPI();
    Q_ASSERT( ibisAPI );
    if( !ibisAPI->GetObjectByID( m_currentProbeObjectId ) ) m_currentProbeObjectId = SceneManager::InvalidId;
    if( m_currentProbeObjectId == SceneManager::InvalidId )
    {
        QList<UsProbeObject *> allProbes;
        ibisAPI->GetAllUsProbeObjects( allProbes );
        if( allProbes.size() > 0 ) m_currentProbeObjectId = allProbes[0]->GetObjectID();
    }

    if( initialProbeId != m_currentProbeObjectId ) emit ObjectsChanged();
}
...
void USAcquisitionPluginInterface::ValidateCurrentVolume()
{
    ...
    IbisAPI * ibisAPI = GetIbisAPI();
    Q_ASSERT( ibisAPI );
    if( !ibisAPI->GetObjectByID( m_currentVolumeObjectId ) ) m_currentVolumeObjectId = SceneManager::InvalidId;
    ...
    if( newVolumeId == SceneManager::InvalidId )
    {
        ...
        if( images.size() > 0 ) newVolumeId = images[0]->GetObjectID();
    }
...
void USAcquisitionPluginInterface::ValidateAddedVolume()
{
    ...
    IbisAPI * ibisAPI = GetIbisAPI();
    Q_ASSERT( ibisAPI );
    if( !ibisAPI->GetObjectByID( m_addedVolumeObjectId ) ) m_addedVolumeObjectId = SceneManager::InvalidId;
...
bool USAcquisitionPluginInterface::CanCaptureTrackedVideo() { return GetCurrentUsProbe() != 0; }
...
void USAcquisitionPluginInterface::SetCurrentAcquisitionObjectId( int id ) { m_currentAcquisitionObjectId = id; }
...
void USAcquisitionPluginInterface::SceneContentChanged() { ValidateAllSceneObjects(); }

void USAcquisitionPluginInterface::LutChanged( int vtkNotUsed( id ) ) { emit ObjectsChanged(); }

void USAcquisitionPluginInterface::OnImageChanged() { emit ImageChanged(); }
