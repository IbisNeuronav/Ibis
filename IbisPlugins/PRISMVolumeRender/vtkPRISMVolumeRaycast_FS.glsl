...
{
    vec4 volSample = texture( volumes[volIndex], pos );
    return volSample.x;
}
...
{
    vec4 volSample = texture( volumes[volIndex], pos );
    vec4 tFuncSample = texture( transferFunctions[volIndex], volSample.x );
    return tFuncSample;
}

vec4 SampleTF( int volIndex, float value )
{
    vec4 tFuncSample = texture( transferFunctions[volIndex], value );
    return tFuncSample;
}

float SampleVolumeSmooth( int volIndex, vec3 pos, float sampleSize )
{
    float volSample = 0.33333 * texture( volumes[volIndex], pos ).x;
    volSample += 0.11111 * texture( volumes[volIndex], pos +  vec3(sampleSize,0.0,0.0)  ).x;
    volSample += 0.11111 * texture( volumes[volIndex], pos +  vec3(0.0, sampleSize, 0.0) ).x;
    volSample += 0.11111 * texture( volumes[volIndex], pos +  vec3(0.0,0.0, sampleSize) ).x;
    volSample += 0.11111 * texture( volumes[volIndex], pos -  vec3(sampleSize,0.0,0.0) ).x;
    volSample += 0.11111 * texture( volumes[volIndex], pos -  vec3(0.0, sampleSize, 0.0) ).x;
    volSample += 0.11111 * texture( volumes[volIndex], pos -  vec3(0.0,0.0,sampleSize) ).x;
    return volSample;
...
vec4 SampleVolumeSmoothWithTF( int volIndex, vec3 pos, float sampleSize )
{
    ...
    vec4 tFuncSample = texture( transferFunctions[volIndex], volSample );
    return tFuncSample;
}
...
{
    vec3 g1;
    g1.x = texture( volumes[volIndex], pos + vec3(gradStep,0.0,0.0)  ).x;
    g1.y = texture( volumes[volIndex], pos +  vec3(0.0, gradStep, 0.0) ).x;
    g1.z = texture( volumes[volIndex], pos +  vec3(0.0,0.0, gradStep) ).x;
    vec3 g2;
    g2.x = texture( volumes[volIndex], pos -  vec3(gradStep,0.0,0.0) ).x;
    g2.y = texture( volumes[volIndex], pos -  vec3(0.0, gradStep, 0.0) ).x;
    g2.z = texture( volumes[volIndex], pos -  vec3(0.0,0.0,gradStep) ).x;
    vec3 n = g2 - g1;
...
void main()
{
    ...
    vec4 depthSample = texture( depthBuffer, gl_FragCoord.xy );
    ...
    ndcPosDepth.w = 1.0;
    vec4 eyeSpaceDepth = projectionMatrixInverse * ndcPosDepth;
