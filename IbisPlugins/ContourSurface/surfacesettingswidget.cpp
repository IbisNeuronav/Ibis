...
SurfaceSettingsWidget::~SurfaceSettingsWidget() { delete ui; }
...
void SurfaceSettingsWidget::SetGeneratedSurface( GeneratedSurface * surface )
{
    ...
    if( m_surface )
    {
        ...
        if( m_surface->IsValid() ) this->SetupHistogramWidget();
    }
}
...
void SurfaceSettingsWidget::ApplyButtonClicked()
{
    ...
    if( ok ) m_surface->SetRadiusFactor( tmp );
    tmp = ui->standardDeviationlineEdit->text().toDouble( &ok );
    if( ok ) m_surface->SetStandardDeviation( tmp );
...
void SurfaceSettingsWidget::SetupHistogramWidget()
{
    ...
    if( lut ) ui->histogramWidget->SetColorTransferFunction( lut->GetColorFunction() );
    this->UpdateUI();
}

void SurfaceSettingsWidget::PolygonReductionSpinBoxValueChanged( int red ) { m_reductionPercent = red; }
...
void SurfaceSettingsWidget::UpdateUI()
{
    ...
    if( ui->histogramWidget->minSliderValue() != m_min ) ui->histogramWidget->setMinSliderValue( m_min );
    if( ui->histogramWidget->maxSliderValue() != m_max ) ui->histogramWidget->setMaxSliderValue( m_max );
}

void SurfaceSettingsWidget::UpdateSettings() { this->UpdateUI(); }
