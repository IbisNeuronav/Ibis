...
void IbisHardwareIGSIO::InternalWriteToolConfig(QString filename, Tool* tool)
{
	...
	{
		vtkSmartPointer<CameraObject> camera = CameraObject::SafeDownCast(tool->sceneObject);
		camera->SerializeTracked(&writer0);
	}
	else if (tool->sceneObject->IsA("UsProbeObject"))
	{
		vtkSmartPointer<UsProbeObject> probe = UsProbeObject::SafeDownCast(tool->sceneObject);
		probe->SerializeTracked(&writer0);
	}
	else if (tool->sceneObject->IsA("PointerObject"))
	{
		vtkSmartPointer<PointerObject> pointer = PointerObject::SafeDownCast(tool->sceneObject);
		pointer->SerializeTracked(&writer0);
	}
	else
	{
